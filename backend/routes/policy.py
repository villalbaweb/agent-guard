...
from ..dependencies import get_policy_engine, require_auth
from ..schemas import PolicyResponse, PolicyReloadResponse

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/policy", tags=["policy"])

# Serializes concurrent POST /policy/reload requests.
...
def reload_policy(auth: Annotated[AuthContext, Depends(require_auth)]) -> PolicyReloadResponse:
    ...
    The reload applies to the shared PolicyEngine used by all executors, so
    subsequent runs immediately pick up the new rules.
